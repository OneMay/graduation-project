...
export default {
  ...
  methods: {
    loginSubmit() {
      ...
      if(mobileReg.test(this.Login.mobile)&&passwordReg.test(this.Login.password)){
    ...
    registerSubmit() {
      const mobileReg=/^[1][3,4,5,7,8][0-9]{9}$/;
      const passwordReg = /(?![0-9]+$)(?![a-zA-Z]+$)[\da-zA-Z]{6,}/;
      if(mobileReg.test(this.Register.mobile)&&passwordReg.test(this.Register.password)){
        this.$Message.success("Success!");
      }else{
        this.$Message.error("手机号或者密码输入有问题");
      }
